...
const getUserWishlist = async () => {
    ...
    if(response){
        console.log(response.data,"djdd")
        return response
    }
}
...
const updateProductFromCart =  async (cartDetail) => {
    const response = await axios.delete(`${base_url}user/update-cart/${cartDetail.cartItemId}/${cartDetail.newQuantity}`,config );
...
const forgetPassToken = async(data) =>{
    const response = await axios.post(`${base_url}user/forget-password-token`,data)
    if (response.data) {
        return response.data
      }
}
