...
export const AddBonCommande= async (order) => {
  ...
  export const  PrixTotal = async (order) => {
    const response = await axios.post( `http://www.24-crm.online/webservice/api/LigBCDV?FAC=${order.numfac}&typfacc=BC&coddos=GEDcom&serveur=/]QPMFD&user=tb&pwd=Qpmztpgubinfenf{hboj3135&database=PSGEDcom`,);
  ...
  export const AddBonCommandeEntéte = async (order) => {
    return await axios.post(`http://www.24-crm.online/webservice/api/BCDVCLIs?numfac&typfac=BC&taurem=0&codcli=${order.codcli}&raisoc=${order.raisoc}&catfisc=0&datfac=${order.datfac}&timbre=false&tauxNASS=0&vendeur=rami&valtimbre=0&codrep=&pj=&smNASS=&0&CodDep=&coddos=GEDcom&serveur=/]QPMFD&user=tb&pwd=Qpmztpgubinfenf{hboj3135&database=PSGEDcom`);
  }
